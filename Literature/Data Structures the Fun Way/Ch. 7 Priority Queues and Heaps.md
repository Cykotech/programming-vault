...
A priority queue assigns priority based on its use case. Some scenarios might be obvious while other others might need to be determined by an algorithm. Network requests can be processed by the age of the initial request or by a static property as part of the data of the request.
